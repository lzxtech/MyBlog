...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | 軒さん🌸")[0];
}
...
$.ajax({
  ...
  data: {
    key: 'UNSBZ-ROHCU-77CVF-GCKCG-DOIB5-FCFP3',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function createtime1() {
  var grt = new Date("20/10/2024 00:00:00");  //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by 🌸軒さん🌸 %c 你正在访问🌸軒さん🌸の小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| 🌸軒さん🌸") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('🌸軒さん🌸的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  // 网站诞生时间
  var grt = new Date("10/20/2024 00:00:00");  // 改成自己的
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<img class='boardsign' src='https://tuchuang.voooe.cn/images/2024/10/26/dba300805e108264b521e9e1332c3982.png' title='科研摸鱼中~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
      : `<img class='boardsign' src='https://tuchuang.voooe.cn/images/2024/10/26/b5b2093e84cf788faaded06586517b49.png' title='下班啦~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
