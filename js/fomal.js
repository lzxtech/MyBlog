...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | 🎊ATRI🎊")[0];
}
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by 🎊ATRI🎊 %c 你正在访问🎊ATRI🎊の小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| 🎊ATRI🎊") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('🎊ATRI🎊的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
